...
[3] Due to the size of this kind of files (one line per genome position), we resume their information by computing a coverage mean per 100bp windows.

[4] For each resumed bam file, we compute a mean coverage and we identify in a masking file, the 100bp window with a mean coverage 2 times higher or lower than the genome mean.
